...
#SBATCH --output=/gpfswork/rech/lco/url46ht/outputs/generic_features_mlp/generic_features_mlp%j_%x_%A_%a.out  # output file name
#SBATCH --error=/gpfswork/rech/lco/url46ht/outputs/generic_features_mlp/generic_features_mlp%j_%x_%A_%a.err   # error file name
#SBATCH --array=0-39
...
DIRNAME=generic_features_mlp

iconfig=$(($SLURM_ARRAY_TASK_ID%8))
config=${base_config[$iconfig]}
name=${base_name[$iconfig]}}

python -m mmsrl.train configs/generic.py --pooling=mlp $config \
